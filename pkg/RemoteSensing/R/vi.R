...
arvi<-function(red,nir,blue) #ARVI: Atmospheric Resistant Vegetation Index
{
...
dvi<-function(red, nir) #DVI: Difference Vegetation Index
{
	result <- (nir - red)
	return(result)
}
...
evi<-function(blue, red, nir) #EVI: Enhanced Vegetation Index 
{
...
gari<-function(red, nir, blue, green) #GARI: green atmospherically resistant vegetation index 
{
...
gemi<-function(red,nir) #GEMI: Global Environmental Monitoring Index
{
...
gvi<-function(blue, green, red, nir, ch5, ch7) #Green Vegetation Index
{
...
ipvi<-function(red, nir) #IPVI: Infrared Percentage Vegetation Index
{
	result <- (nir) / (nir + red)
	return(result)
}
...
msavi2<-function(red, nir) #MSAVI2: second Modified Soil Adjusted Vegetation Index
{
...
msavi<-function(red, nir) #MSAVI: Modified Soil Adjusted Vegetation Index
{
...
pvi<-function(red, nir) #PVI: Perpendicular Vegetation Index 
{
...
savi<-function(red, nir) #SAVI: Soil Adjusted Vegetation Index
{
...
sr<-function(red, nir) #SR: Simple Vegetation ratio
{
	result <- (nir / red)
	return(result)
}
...
vari<-function(blue, green, red) #VARI: Visible Atmospherically Resistant Index
{
...
wdvi<-function(red, nir, slslope = 1) #WDVI: Weighted Difference Vegetation Index
{
